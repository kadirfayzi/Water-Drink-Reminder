...
class _CupertinoTabBarState extends State<CupertinoTabBar> with SingleTickerProviderStateMixin {
  ...
  bool get _shouldBeScrollable =>
      !widget.allowExpand && widget.allowScrollable && _preferredWidth! > _fullWidth!;
  ...
  bool get _shouldBeReanimated =>
      widget.animateUntilScrolled && !_userHasScrolled && _animationController!.isCompleted;
  ...
  double get _offset {
    ...
    double _tabStartingPoint =
        widget.outerHorizontalPadding / 2.0 + _singleWidth * widget._valueGetter();
    ...
    _targetOffset = _targetOffset.clamp(
        0.0, _preferredWidth! - _fullWidth! + widget.outerHorizontalPadding * 2);
    return _targetOffset;
  }

  bool get _isRTL => Directionality.of(context) == TextDirection.rtl;
  Alignment get _alignment => Alignment(
      (_isRTL ? -1 : 1) * (-1.0 + widget._valueGetter() / (widget._widgets.length - 1) * 2), 0.0);

  void _startAnimation() {
    _animationTimer =
        Timer(const Duration(seconds: 4), () => _animationController!.forward(from: 0.0));
  }
  ...
  void _applyAnimation() {
    if (_animationController!.value < _tresholds[0]) {
      ...
      _scrollController!.jumpTo(
          _initialScrollOffset + _tresholds[0] - (_animationController!.value - _tresholds[0]));
    } else if (_animationController!.value >= _tresholds[1] &&
        _animationController!.value < _tresholds[2]) {
      _scrollController!.jumpTo(
          _initialScrollOffset - _tresholds[0] + (_animationController!.value - _tresholds[1]));
    } else if (_animationController!.value >= _tresholds[2] &&
        _animationController!.value < _animationController!.upperBound * 0.95) {
      _scrollController!.jumpTo(
          _initialScrollOffset + _tresholds[0] - (_animationController!.value - _tresholds[2]));
    } else {
    ...
    if (!_showSelf) {
      return Stack(
        children: List<Widget>.generate(
            _shouldCalculateFullDimensions ? widget._widgets.length + 1 : widget._widgets.length,
            (int index) {
      ...
      Widget _widget = Stack(
        ...
        children: [
          ...
          Row(
            children: List<Widget>.generate(
                widget.useSeparators ? widget._widgets.length * 2 - 1 : widget._widgets.length,
                (int index) {
      ...
      if (_shouldBeScrollable) {
        ...
        if (_shouldBeAnimated) {
          if (_animationController == null) {
            ...
            _animationTimer =
                Timer(const Duration(seconds: 2), () => _animationController!.forward(from: 0.0));
            _tresholds = [
...
class _Indicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedAlign(
      ...
      child: Padding(
        padding: _useShadow
            ? const EdgeInsets.only(left: 2.0, top: 2.0, right: 3.0, bottom: 4.0)
